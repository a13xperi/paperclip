...
/**
 * The claude_api adapter writes structured JSON lines that mirror the
 * claude_local CLI's stream format (system/init, assistant, result) so the
 * existing transcript viewer can render runs unchanged. We intentionally
 * reuse the shape instead of introducing a new transcript kind.
 */
